...
| 控制項 | 說明 |
|--------|------|
| **日期區間** | `DatePickerRange`，所有圖表與表格依時間窗動態切片；**預設最近 30 天**，讓環比一開啟就有前期可比 |
...
- **前十大熱銷商品（已售數量）**
- **已購買商品後端分頁明細表**：已濾除整列去識別化的 `(not set)` 垃圾品項（約 2.8%）

### 效能計時器
頂部右側 Badge 在每次查詢後**出現約 1 秒即自動淡出**（clientside 控制），顯示該次 DuckDB 耗時與掃描筆數，例如 `⚡ DuckDB 查詢 743 ms · 掃描 1,188,051 筆`，不長駐畫面。

> **關於環比 (vs 前一個月/季/年)**：環比拿「目前區間」對比「**等長且緊鄰其前**的區間」，並依長度自動命名——選約 30 天→「前一個月」、約 90 天→「前一季」、約 365 天→「前一年」。KPI 列下方會註明實際比較的日期。預設顯示最近 30 天，前期自然有資料；若把起始日拉到資料最早日 (2020-11-01)，前期落在資料範圍外，會顯示「無前期可比」，屬正常。
>
