...
### 效能計時器
頂部右側 Badge 持續顯示最近一次查詢的 DuckDB 耗時與掃描筆數，例如 `⚡ DuckDB 查詢 743 ms · 掃描 1,188,051 筆`。

> **關於環比 (vs 前期)**：環比拿「目前區間」對比「等長且緊鄰其前的區間」。預設顯示最近 30 天，前期自然有資料；若你把起始日拉到資料最早日 (2020-11-01)，前期落在資料範圍外，會顯示「無前期可比」，屬正常。
>
