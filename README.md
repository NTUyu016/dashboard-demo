...
> DuckDB + Plotly Dash · 100% 地端 (Air-gapped) · 資料源：Google GA4 公開電商樣本，同構類比為「廠內物料與採購生命週期日誌 (PO Lifecycle Log)」。
...
- **無外部 DB Server、無外部 API**：DuckDB 直接掃描本地 Parquet，全程地端。
...
**產出檔案：**
- `events_data.parquet`：事件維度主表（Snappy 壓縮）
- `products_data.parquet`：商品維度明細表（Snappy 壓縮）
...
> 延伸驗證：本儲存庫的 [`duckdb-poc/`](../duckdb-poc/) 進一步比較了 **DuckDB（欄式）vs SQLite（列式）** 在千萬～億級資料上的 OLAP 聚合效能，DuckDB 快 **50–110 倍**（詳見該資料夾 README 與 `timings.png`）。本儀表板「Parquet + DuckDB」的組合即建立在這個結論之上。
...
| 控制項 | 說明 |
|--------|------|
| **日期區間** | `DatePickerRange`，所有圖表與表格依時間窗動態切片 |
| 流量媒介篩選 | 多選下拉，對應 GA4 `traffic_medium` |
| 關鍵字搜尋 | 同時搜尋活動名稱、來源、國家 |
| 重設按鈕 | 清空所有篩選條件（含日期還原為全區間） |

### 事件分析 Tab
- **KPI 卡片（含環比）**：事件總數 / 不重複使用者 / 收益 / 交易筆數，下方顯示 ▲▼ vs 前期（等長緊鄰區間）變化率
- **每日趨勢圖**：事件量長條 + **7 日移動平均線** + 採購金額折線，雙 Y 軸
- **轉換漏斗**：瀏覽商品 → 加入購物車 → 進入結帳 → 完成交易，附各階段轉換率與整體轉換率
- **裝置類型圓餅圖**
- **後端分頁明細表**：每頁 20 筆，`LIMIT / OFFSET` 精準回傳，零卡頓

### 商品分析 Tab
- **前五大商品類別營收**：即時跨表 JOIN（events × products）
- **前十大熱銷商品（數量）**
- **後端分頁商品明細表**

### 效能計時器
頂部 Badge 顯示每次 Callback 的 DuckDB 掃描耗時（毫秒），驗證地端大數據效能。
...
| # | 功能 | 技術機制 |
|---|------|----------|
| 1 | **毫秒級效能計時器** | `time.perf_counter()` 量測每次 DuckDB 掃描+運算耗時，頂部 Badge 高亮顯示 |
| 2 | **跨表動態聯查** | 篩選時 DuckDB 直接對兩個 Parquet 執行 `JOIN`，算出前五大採購科目經費 |
| 3 | **後端分頁表** | DataTable `custom` 模式，`LIMIT N OFFSET M` 每次精準只回 20 筆 |
| 4 | **轉換漏斗 + 環比** | 單次掃描算出漏斗各階段事件數與轉換率；KPI 自動比對等長前期，呈現 ▲▼ 變化 |
...
| GA4 原始欄位 | 採購場景類比 |
|--------------|-------------|
| `event_name`（page_view → purchase） | 採購節點（PR_Created → Good_Received） |
| `geo.country` | 全球供應商採購支出佔比 |
| `device.category` | 廠區 / 終端設備分類 |
| `traffic_medium` | 採購單進件管道 |
| `item_category` | 採購經費科目 |
| `transaction_id` | 採購單號 PO Number |
